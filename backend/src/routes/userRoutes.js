...
router.post("/", authenticate, authorize(["admin"]), async (req, res) => {
  ...
  try {
    ...
    if (userError) {
      ...
      return res.status(400).json({
        message: userError.message || "Error creating user (users insert)",
      });
    }
...
router.put("/:id", authenticate, authorize(["admin"]), async (req, res) => {
  ...
  try {
    ...
    if (updateError) {
      ...
      return res.status(400).json({
        message: updateError.message || "Error updating user (users update)",
      });
    }
...
router.delete("/:id", authenticate, authorize(["admin"]), async (req, res) => {
  ...
  try {
    ...
    if (error) {
      ...
      return res.status(400).json({
        message: error.message || "Error deleting user (users delete)",
      });
    }
