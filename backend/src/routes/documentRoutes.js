    ...
    try {
      ...
      if (insertError) {
        ...
        return res.status(400).json({
          message: insertError.message || "Error creating document",
        });
      }
...
router.put(
  ...
  async (req, res) => {
    ...
    const { title, category, description, file_url, visibility } = req.body;
    ...
    try {
      ...
      if (updateError) {
        ...
        return res.status(400).json({
          message: updateError.message || "Error updating document",
        });
      }
