  ...
  async (req, res) => {
    try {
      ...
      if (error) {
        ...
        return res.status(500).json({ message: "Error fetching payroll runs" });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (error) {
        ...
        return res.status(500).json({ message: "Error creating payroll run" });
      }
...
router.get("/runs/:id", authenticate, authorize([]), async (req, res) => {
  try {
    ...
    if (itemsError) {
      ...
      return res.status(500).json({ message: "Error fetching payroll items" });
    }
