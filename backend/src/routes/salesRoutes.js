  ...
  async (req, res) => {
    try {
      ...
      if (leadsError) {
        ...
        return res
          .status(500)
          .json({ message: "Error loading leads for sales summary" });
      }
      ...
      if (paymentsError) {
        ...
        return res
          .status(500)
          .json({ message: "Error loading payments for sales summary" });
      }
      ...
      const { data: recentPaymentsRaw, error: recentPaymentsError } =
        await supabase
          .from("payments")
          .select(
            `
      ...
      if (recentPaymentsError) {
        console.error(
          "Sales summary recent payments error:",
          recentPaymentsError,
        );
      }
