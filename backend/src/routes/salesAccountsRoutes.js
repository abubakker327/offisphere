...
const postLedgerEntries = async (entries = []) => {
  ...
  if (!ensureBalanced(entries)) {
    return { ok: false, message: 'Ledger not balanced (debit/credit mismatch)' };
  }
...
router.get('/masters/products', authenticate, authorize([]), async (_req, res) => {
...
router.post('/masters/products', authenticate, authorize([]), async (req, res) => {
  const {
    name,
    sku,
    category,
  ...
  const insertPayload = {
    ...
    has_serial: typeof has_serial === 'boolean' ? has_serial : !!is_serialized,
...
router.get('/masters/vendors', authenticate, authorize([]), async (_req, res) => {
...
router.post('/masters/vendors', authenticate, authorize([]), async (req, res) => {
  ...
  if (error) {
    ...
    return res.status(500).json({ message: error.message || 'Error creating vendor' });
  }
  res.status(201).json({ message: 'Vendor created' });
});

router.get('/masters/customers', authenticate, authorize([]), async (_req, res) => {
...
router.post('/masters/customers', authenticate, authorize([]), async (req, res) => {
  const { name, email } = req.body || {};
  if (!name) {
    return res.status(400).json({ message: 'Name is required' });
  }
...
router.post('/procurement/po', authenticate, authorize([]), async (req, res) => {
  try {
    const { vendor_id, warehouse_id, items = [], status = 'draft', notes } = req.body || {};
    if (!vendor_id || !items.length) {
      return res.status(400).json({ message: 'vendor_id and items are required' });
    }
    ...
    const { error: itemError } = await supabase.from('purchase_order_items').insert(itemsPayload);
    if (itemError) {
      console.error('PO items insert error:', itemError);
      return res.status(500).json({ message: 'Error creating purchase order items' });
    }
...
router.get('/procurement/po', authenticate, authorize([]), async (_req, res) => {
...
router.get('/procurement/po/:id/items', authenticate, authorize([]), async (req, res) => {
  const { id } = req.params;
  const { data, error } = await supabase
  ...
  if (error) {
    ...
    return res.status(500).json({ message: 'Error fetching purchase order items' });
  }
  res.json(data || []);
});

router.post('/procurement/grn', authenticate, authorize([]), async (req, res) => {
  try {
    const { po_id, items = [], received_by, remarks } = req.body || {};
    if (!po_id || !items.length) {
      return res.status(400).json({ message: 'po_id and items are required' });
    }
    ...
    const { error: itemsError } = await supabase.from('grn_items').insert(itemsPayload);
    if (itemsError) {
...
router.post('/sales/quotation', authenticate, authorize([]), async (req, res) => {
  try {
    ...
    const { error: itemError } = await supabase.from('sales_quotation_items').insert(itemsPayload);
    if (itemError) {
      console.error('Quotation items error:', itemError);
      return res.status(500).json({ message: 'Error creating quotation items' });
    }
...
// List quotations
router.get('/sales/quotation', authenticate, authorize([]), async (req, res) => {
...
router.post('/sales/order', authenticate, authorize([]), async (req, res) => {
  try {
    const { quotation_id, customer_id, items = [], status = 'draft', notes } = req.body || {};
    if (!customer_id || !items.length) {
      return res.status(400).json({ message: 'customer_id and items are required' });
    }
    const totals = computeTotals(items);
    ...
    if (itemError) {
      ...
      return res.status(500).json({ message: 'Error creating sales order items' });
    }
...
router.post('/sales/delivery', authenticate, authorize([]), async (req, res) => {
  try {
    const { sales_order_id, warehouse_id, items = [], notes } = req.body || {};
    if (!sales_order_id || !items.length) {
      return res.status(400).json({ message: 'sales_order_id and items are required' });
    }
    ...
    if (!uuidRegex.test(sales_order_id)) {
      const { data: soRow, error: soError } = await supabase
        ...
        .single();
      if (soError || !soRow) {
        return res.status(400).json({ message: 'Invalid sales_order_id/so_number' });
      }
    ...
    if (error) {
      ...
      return res.status(500).json({ message: 'Error creating delivery challan' });
    }
...
router.post('/sales/invoice', authenticate, authorize([]), async (req, res) => {
  try {
    ...
    const entries = [
      { ledger: 'AR_Customer', debit: totals.total, credit: 0, ref_type: 'INVOICE', ref_id: row.id },
      { ledger: 'Revenue', debit: 0, credit: totals.subtotal, ref_type: 'INVOICE', ref_id: row.id },
      { ledger: 'GST_Payable', debit: 0, credit: totals.gst_total, ref_type: 'INVOICE', ref_id: row.id }
    ];
    const ledgerResult = await postLedgerEntries(entries);
    if (!ledgerResult.ok) {
      return res.status(500).json({ message: ledgerResult.message });
...
router.post('/payments/in', authenticate, authorize([]), async (req, res) => {
  try {
    const { invoice_id, customer_id, amount, mode, currency = 'INR', date, notes } = req.body || {};
    ...
    if (!uuidRegex.test(invoice_id)) {
      const { data: invRow, error: invErr } = await supabase
        ...
        .single();
      if (invErr || !invRow) {
        return res.status(400).json({ message: 'Invalid invoice_id/invoice_number' });
      }
    ...
    if (!resolvedCustomerId) {
      return res.status(400).json({ message: 'customer_id is required (provide or inferred from invoice)' });
    }
    const paymentMethod = mode && String(mode).trim() ? String(mode).trim() : 'cash';
    ...
    const entries = [
      { ledger: 'Cash_Bank', debit: safeNumber(amount), credit: 0, ref_type: 'PAYMENT_IN', ref_id: row.id },
      { ledger: 'AR_Customer', debit: 0, credit: safeNumber(amount), ref_type: 'PAYMENT_IN', ref_id: row.id }
    ];
    const ledgerResult = await postLedgerEntries(entries);
    if (!ledgerResult.ok) {
      return res.status(500).json({ message: ledgerResult.message });
...
router.post('/payments/out', authenticate, authorize([]), async (req, res) => {
  try {
    const { vendor_id, po_id, grn_id, amount, mode, currency = 'INR', date, notes } = req.body || {};
    if (!vendor_id || !amount) {
    ...
    if (error) {
      ...
      return res.status(500).json({ message: 'Error recording vendor payment' });
    }

    const entries = [
      { ledger: 'AP_Vendor', debit: safeNumber(amount), credit: 0, ref_type: 'PAYMENT_OUT', ref_id: row.id },
      { ledger: 'Cash_Bank', debit: 0, credit: safeNumber(amount), ref_type: 'PAYMENT_OUT', ref_id: row.id }
    ];
    const ledgerResult = await postLedgerEntries(entries);
    if (!ledgerResult.ok) {
      return res.status(500).json({ message: ledgerResult.message });
...
router.get('/accounting/ledger', authenticate, authorize([]), async (req, res) => {
