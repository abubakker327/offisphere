-- Fix: Remove foreign key constraint from events.created_by
-- This allows events to be created with user IDs that don't exist in auth.users

-- Drop the existing events table if it exists with the foreign key
DROP TABLE IF EXISTS events CASCADE;

-- Recreate the events table without the foreign key constraint
